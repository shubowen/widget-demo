...
public class SwipeMenuLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(contentWidth, MeasureSpec.EXACTLY);
...
/*                    if (!mScroller.isFinished()) {
    ...
    /**
     * 找到RecyclerView或者AbsListView里面的其他的展开的SwipeMenuLayout并关闭
     *
     * @return
     */
    private boolean findAndCloseOthers() {
        boolean find = false;
        ViewParent parent = getParent();
        if (null != parent && (parent instanceof RecyclerView || parent instanceof AbsListView)) {
            ViewGroup viewGroup = (ViewGroup) parent;
            int childCount = viewGroup.getChildCount();
            for (int i = 0; i < childCount; i++) {
                View child = viewGroup.getChildAt(i);
                if (child instanceof SwipeMenuLayout) {
                    SwipeMenuLayout another = (SwipeMenuLayout) child;
                    if (another != this && another.isExpand) {
                        another.smoothClose();
                        find = true;
                    }
                }
